...
- [ ] 1. **Настроить переменные окружения**: Добавить `OWNER_TELEGRAM_ID` и `ADMIN_TELEGRAM_ID` в файл `.env` (будут загружены `config.py`).
- [ ] 2. **Обновить структуру Google Sheets**:
    - [ ] Создать лист `Пользователи` с колонками: `telegram_id`, `телефон`, `фио`, `автоколонна`, `статус`.
    - [ ] Создать лист `Кампании` с колонками: `название_кампании`, `дедлайн`, `тип`, `тип_назначения`, `значение_назначения`.
    - [ ] В лист `Вопросы` добавить колонки `критический_вопрос` (Boolean) и `пояснение` (Text).
    - [ ] В лист `Результаты` добавить колонки `итоговый_статус` (Text) и `название_кампании` (Text).
