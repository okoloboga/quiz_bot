...
В файл `.env` будут добавлены следующие переменные:
...
Структура листов Google Sheets будет расширена для поддержки нового функционала, как описано в предыдущей версии плана (новые листы `Users`, `Campaigns` и новые колонки в `Questions`, `Results`).
...
- **Логика**:
    ...
    2.  Выполняется запрос к листу `Users` для получения статуса этого пользователя.
    ...
    4.  **Если пользователь не найден** в листе `Users`, запускается флоу регистрации.
...
- **Логика**:
    ...
    5.  **Запись в Google Sheets**: Вызывается функция сервисного слоя, которая добавляет новую строку со всеми собранными данными и статусом "ожидает" в лист `Users`.
...
- **Функция `get_active_campaign_for_user(user_id)`**:
    ...
    - **Логика**:
        1.  Получить данные пользователя (`full_name`, `motorcade`) из листа `Users`.
        2.  Получить список всех кампаний из листа `Campaigns`.
        3.  Отфильтровать кампании, у которых `deadline` уже прошел.
        4.  Пройтись по оставшимся кампаниям и определить, является ли пользователь участником, сверяя `assignment_type` и `assignment_value` с данными пользователя.
        5.  Для каждой подходящей кампании проверить лист `Results`: если для пары `user_id` и `campaign_name` есть запись со статусом "пройдено", эта кампания игнорируется.
...
- **Хендлер команды `/start` (для подтверждённых пользователей)**:
    ...
    2.  Если кампания найдена, показывает пользователю Inline-кнопку "Начать" с `callback_data`, содержащим `campaign_name`.
...
- **Хендлер `callback_query` (нажатие кнопки "Начать")**:
    1.  Извлекает `campaign_name`.
    2.  Проверяет в `Results` статус последней попытки для этой кампании. Если статус "не пройдено", то проверяет, есть ли отметка "разрешена пересдача".
    3.  Если доступ разрешен, запускает процесс подготовки к тесту: загружает вопросы, формирует сессию в Redis, сохраняя в ней `campaign_name` и `type` (`Обучение`/`Тестирование`).
...
- **Хендлер ответа на вопрос**:
    ...
    - **Логика**:
        ...
        2.  **Если ответ НЕПРАВИЛЬНЫЙ**:
            - Загружает данные текущего вопроса, включая `is_critical` и `explanation`.
            - Проверяет, является ли вопрос критическим (`is_critical == TRUE`). Если да, немедленно вызывает функцию `finish_test` с результатом "Не пройден".
            - Если вопрос не критический, уменьшает счётчик оставшихся ошибок.
            - Проверяет контекст теста в сессии Redis. Если режим `Обучение`, отправляет пользователю сообщение с текстом из колонки `explanation`.
...
- **Функция `finish_test(session_data, result)`**:
    ...
    - **Логика**:
        1.  Формирует запись для `Results` на основе данных из сессии (включая `campaign_name`).
...
- **Фоновая задача `check_deadlines_job()` (запускается раз в сутки)**:
    ...
    3.  Итерируется по кампаниям. Если до дедлайна 3 дня или 1 день:
        ...
        - Для каждого участника проверяет, есть ли у него в `Results` статус "пройдено" по этой кампании.
